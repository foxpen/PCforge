...
const PALETTES = [
  { id: 'indigo', label: 'Indigo', accent: '#818cf8', soft: 'rgba(129,140,248,0.15)', border: 'rgba(129,140,248,0.35)', light: '#6366f1' },
  { id: 'violet', label: 'Violet', accent: '#a78bfa', soft: 'rgba(167,139,250,0.15)', border: 'rgba(167,139,250,0.35)', light: '#7c3aed' },
  { id: 'cyan',   label: 'Cyan',   accent: '#22d3ee', soft: 'rgba(34,211,238,0.15)',  border: 'rgba(34,211,238,0.35)',  light: '#0891b2' },
  { id: 'rose',   label: 'Rose',   accent: '#fb7185', soft: 'rgba(251,113,133,0.15)', border: 'rgba(251,113,133,0.35)', light: '#e11d48' },
]
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <button
            ...
            style={{color:'var(--tx)', border:`1.5px solid ${paletteOpen ? 'var(--accent)' : 'var(--glass-b)'}` }}
          >🎨</button>
          ...
          {paletteOpen && (
            <>
              ...
              <div className="absolute right-0 top-12 z-50 rounded-2xl p-3 flex flex-col gap-1.5 shadow-xl"
                style={{background:'rgba(13,13,26,0.97)', border:'1px solid var(--glass-b)', backdropFilter:'blur(24px)', minWidth:'160px'}}>
                <div className="text-[0.6rem] font-bold uppercase tracking-widest px-1 mb-1" style={{color:'var(--tx3)'}}>Barevné téma</div>
                {PALETTES.map(p => (
                  <button
                    key={p.id}
                    onClick={() => { onPalette(p.id); setPaletteOpen(false) }}
                    className="flex items-center gap-2.5 px-3 py-2 rounded-xl cursor-pointer border transition-all text-left w-full"
                    style={{
                      background: palette === p.id ? p.soft : 'transparent',
                      borderColor: palette === p.id ? p.border : 'transparent',
                      color: 'var(--tx)',
                    }}
                  >
