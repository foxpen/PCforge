...
export default function LegoScene({ sel = {}, activeCat = null }) {
  ...
  // Inicializace scény — jen jednou
  useEffect(() => {
    const container = mountRef.current
    if (!container) return

    const cleanup = buildScene(container, api => {
...
// ─────────────────────────────────────────────────────────
function buildScene(container, onReady) {
  ...
  ;[[-8.4, -5.4], [8.4, -5.4], [-8.4, 5.4], [8.4, 5.4]].forEach(([x, z]) => {
    ...
    leg.position.set(x * .95, -1.74, z * .95); desk.add(leg)
  })
  ...
  const glassMat = new THREE.MeshStandardMaterial({ color: 0x88aaff, transparent: true, opacity: 0.13, roughness: 0, metalness: 0.15 })
  const glass = new THREE.Mesh(new THREE.BoxGeometry(0.07, 5.6, 2.2), glassMat)
  glass.position.set(-1.55, 2.9, 0); pcCase.add(glass)

  ;[1.4, 3.4].forEach(y => {
    const ff = legoBrick(1.0, 0.1, 1.0, 0x1a1a24, 1, 1)
    ff.position.set(0, y, 1.28); pcCase.add(ff)
  })

  const pwrLed = new THREE.Mesh(
    new THREE.CylinderGeometry(0.1, 0.1, 0.06, 10),
    new THREE.MeshStandardMaterial({ color: 0x00ff88, emissive: new THREE.Color(0x00ff88), emissiveIntensity: 0.5 })
  )
  ...
  ;[0xff2222, 0x2222ff, 0x22ff22, 0xff8800].forEach((c, i) => {
    const strip = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: c, emissive: new THREE.Color(c), emissiveIntensity: 0 })
    )
    strip.position.set(-1.48, 0.9 + i * 1.1, 0)
    pcCase.add(strip); rgbStrips.push(strip)
  })

  // ── CÍLE UVNITŘ SKŘÍNĚ ──
  const TARGETS = {
    mb:   { x: 0,    y: 1.8,  z: 0,    ry: 0 },
    cpu:  { x: 0.1,  y: 2.55, z: 0.1,  ry: 0 },
    cool: { x: 0.1,  y: 3.35, z: 0.1,  ry: 0 },
    gpu:  { x: 0,    y: 1.8,  z: 0,    ry: Math.PI / 2 },
    ram:  { x: 0.55, y: 2.55, z: 0.05, ry: 0 },
    ssd:  { x: 0.2,  y: 1.4,  z: 0.2,  ry: 0 },
    psu:  { x: 0,    y: 0.55, z: 0,    ry: 0 },
  }

  // ── STARTOVNÍ POZICE NA STOLE ──
  const DESK_POS = {
    mb:   new THREE.Vector3(-4.5, 0.15, -1.5),
    cpu:  new THREE.Vector3(-2.2, 0.15,  2.8),
    gpu:  new THREE.Vector3(-4.5, 0.15,  2.0),
    ram:  new THREE.Vector3( 0.2, 0.15,  3.2),
    cool: new THREE.Vector3( 0.8, 0.15,  1.2),
    psu:  new THREE.Vector3( 3.0, 0.15, -2.5),
    ssd:  new THREE.Vector3( 1.8, 0.15,  3.0),
  }
  ...
  const MAKE = {
    ...
    ram: () => {
      ...
      ;[-0.33, 0.33].forEach((x, i) => {
        const s = legoBrick(0.24, 0.75, 1.8, i ? 0x1a8833 : 0x22aa44, 1, 2)
        s.position.set(x, 0.375, 0); g.add(s)
      })
      return g
    },
    cool: () => {
      const g = new THREE.Group()
      for (let i = 0; i < 7; i++) {
        const f = mbox(0.95, 1.3, 0.05, mat(0xaaaacc, 0.35, 0.65))
        f.position.set(0, 0.65, -0.28 + i * 0.095); g.add(f)
      }
    ...
    ssd: () => {
      ...
      ;[0.38, 0, -0.38].forEach(x => {
        const c = mbox(0.27, 0.065, 0.26, mat(0x111111, 0.2, 0.8)); c.position.set(x, 0.105, 0); g.add(c)
      })
      return g
    },
  }
  ...
  const mountComponent = (key, instant = false) => {
    ...
    const fromP = g.position.clone(), toP = getTargetWorld(key)
    const fromRY = g.rotation.y, toRY = TARGETS[key]?.ry || 0
    if (instant) {
      g.position.copy(toP); g.rotation.set(0, toRY, 0)
      compState[key] = 'inside'; updateRgb(); return
    }
    animations[key] = { t: 0, dur: 0.75, fromP, toP, fromRY, toRY, onDone: () => { compState[key] = 'inside'; updateRgb() } }
  }
  ...
  const unmountComponent = key => {
    ...
    const fromP = g.position.clone(), toP = DESK_POS[key].clone()
    const fromRY = g.rotation.y
    animations[key] = { t: 0, dur: 0.65, fromP, toP, fromRY, toRY: 0, onDone: () => { compState[key] = 'desk'; updateRgb() } }
  }
  ...
  const animate = () => {
    ...
    Object.entries(animations).forEach(([key, anim]) => {
      ...
      if (g) {
        ...
        g.position.y += Math.sin(p * Math.PI) * 4.0
        g.rotation.y = anim.fromRY + (anim.toRY - anim.fromRY) * e + Math.sin(p * Math.PI) * 0.8
      }
      if (p >= 1) {
        anim.onDone?.(); delete animations[key]
        if (g) {
          if (compState[key] === 'inside') {
            g.position.copy(getTargetWorld(key)); g.rotation.set(0, TARGETS[key]?.ry || 0, 0)
          } else {
            g.position.copy(DESK_POS[key]); g.rotation.set(0, 0, 0)
          }
        }
      }
    })
